...
import { LEARNING_BLOCKS, availableBlocks, blockPool, nextBlockItem, nextBlockTarget,
         summariseLearningBlock, toLearningTrials } from '@/lib/exam/phase2';

function rng(s: number) { return () => (s = (s * 1103515245 + 12345) % 2147483648) / 2147483648; }
  ...
  it('null cohort per type', async () => {
    ...
    for (const spec of availableBlocks(pool, [], standings)) {
      ...
      for (let child = 0; child < 40; child += 1) {
        ...
        const admin: string[] = []; const trials: { difficulty: number; score: number }[] = [];
        for (let t = 0; t < spec.length; t += 1) {
          const item = nextBlockItem(sub, admin, nextBlockTarget(toLearningTrials(trials), 12), t + 1);
          if (!item) break;
        ...
        if (r.lambda !== null) { sum += r.lambda; n += 1; }
      }
      out.push(`  ${spec.typeCode.padEnd(20)} options=${opts} floor=${floor.toFixed(3)} null_lambda=${(sum / n).toFixed(4)}`);
    }
    throw new Error(`\nFLOORPROBE\n${out.join('\n')}\n`);
  });
});
