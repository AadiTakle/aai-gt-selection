...
describe('P', () => { it('p', async () => {
  const pool = await getServedIndex(); const p1 = phase1Pool(pool);
  const by = (arr: typeof pool) => ['fluid_reasoning','verbal','quantitative','spatial']
    .map(a => `${a.slice(0,4)}=${arr.filter(i=>i.domain===a).length}`).join(' ');
