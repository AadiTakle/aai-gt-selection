...
export const COARSE_LURE_CLASS = Object.freeze({
  // --- argument evaluation and situational judgement -----------------------
  // GB-DEBATE-01, CX-sjt-01 and CX-curious-02 label by how the option fails the
  // task rather than by geometry, so the mapping reads: violating a stated
  // constraint is a rule violation, meeting some of it is a local fit, missing
  // the point entirely is a global mismatch, and supporting the other side is
  // the relation reversed. Three fallacy labels have no honest home here and
  // are in NO_HONEST_COARSE_CLASS instead.
  breaks_a_stated_requirement: 'rule_violation',
  too_late_for_the_deadline: 'rule_violation',
  meets_only_some_requirements: 'local_fit',
  weaker_but_relevant: 'local_fit',
  single_case_anecdote: 'local_fit',
  ignores_the_stated_goal: 'global_mismatch',
  off_topic: 'global_mismatch',
  consistent_with_rival: 'reversed_relation',
  true_but_irrelevant: 'associate',
  thematic_associate: 'associate',
  // Already answered by the scene, so it reveals no information gap: it echoes
  // a visible surface feature instead of finding what is missing.
  stated_in_scene: 'surface_match',
  stated_in_later_line: 'surface_match',
  ...
  appeal_to_popularity: 'distractor_other',
  attacks_the_person: 'distractor_other',
  blames_a_person: 'distractor_other',
 ...
 *                        structure
 *   informal-fallacy     a named reasoning fallacy (ad hominem, bandwagon).
 *                        The argument-evaluation types treat the fallacy itself
 *                        as the thing being tested, and the enum describes how
 *                        an answer relates to a key, not how an argument fails.
 */
export const NO_HONEST_COARSE_CLASS = Object.freeze({
  appeal_to_popularity: 'informal-fallacy',
  attacks_the_person: 'informal-fallacy',
  blames_a_person: 'informal-fallacy',
...
function normalizeRationale(entry) {
  ...
  if (!labelled) {
    // A multi-decision type nests one rationale map per decision, so an
    // unlabelled entry whose every value is itself a rationale is a grouping
    // level rather than a malformed entry. GB-DEBATE-01 keys two independent
    // decisions this way (`{support: {e1: …}, rebut: {e1: …}}`).
    const values = Object.values(entry);
    const isGroup =
      values.length > 0 &&
      values.every((v) => typeof v === 'string' || (v && typeof v === 'object' && !Array.isArray(v)));
    if (isGroup) {
      const nested = {};
      for (const [k, v] of Object.entries(entry)) nested[k] = normalizeRationale(v);
      return nested;
    }
