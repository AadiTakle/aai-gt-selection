// Independent validator for the VER-RELPAIR-01 structured bank.
//
...
//
// Run:  node research/exam-question-types/generators/check-VER-RELPAIR-01.mjs

import { readFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { buildBank } from './VER-RELPAIR-01.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const BANK = resolve(__dirname, '../banks/VER-RELPAIR-01.jsonl');

const TYPE = 'VER-RELPAIR-01';
const DOMAIN = 'verbal';
...
const LEAK_KEY = /^(lure|lures|misconception|correct|iscorrect|is_correct|correctkey|key|keys|answer|answers|answerkey|solution|solver|rationale|rationales|distractorrationales|fit|why|note|explanation|error|errortype|error_type|truth|verdict|valid|isvalid|target|expected|score|points)$/i;
...
for (const it of items) {
  ...
  if (typeof c.relation !== 'string' || !c.relation) fail(id, 'content.relation missing');
  if (!Array.isArray(c.stemPair) || c.stemPair.length !== 2 || c.stemPair.some((t) => !t || typeof t.text !== 'string')) fail(id, 'content.stemPair must be 2 tokens with text');
  ...
  opts.forEach((o, oi) => {
    if (!deepEq(Object.keys(o).sort(), ['pair'])) fail(id, `option[${oi}] carries fields beyond 'pair' (${Object.keys(o)})`);
    if (!Array.isArray(o.pair) || o.pair.length !== 2 || o.pair.some((t) => !t || typeof t.text !== 'string')) fail(id, `option[${oi}] pair must be 2 tokens with text`);
    o.pair.forEach((t) => { if (!deepEq(Object.keys(t).sort(), ['text'])) fail(id, `option[${oi}] token carries extra fields (${Object.keys(t)})`); });
  });
  const sigs = opts.map((o) => o.pair.map((t) => t.text).join('~'));
  if (new Set(sigs).size !== sigs.length) fail(id, 'duplicate option pairs');
...
// For every field appearing on option objects, and for the raw option payload itself,
// check whether any value is a reliable "this one is correct" tell across the bank.
{
