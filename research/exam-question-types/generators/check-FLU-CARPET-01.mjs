// Tiny validator for the FLU-CARPET-01 structured bank.
//
...
//   3. Answer key is valid, unique, and matches an INDEPENDENT series solver
//      (re-derive each progression from the VISIBLE carpet cells, continue it into
//      the blank, confirm exactly one option matches and it is the declared key).
...
//
// Run:  node research/exam-question-types/generators/check-FLU-CARPET-01.mjs

import { readFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { genItem, difficultyFromLevers, LINEAR_ATTRS } from './FLU-CARPET-01.mjs';
import { lureLabel, normalizeBankItem } from './item-shape.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const BANK = resolve(__dirname, '../banks/FLU-CARPET-01.jsonl');
const ALLOWED_BANDS = ['K-1', '2-3', '4-5', '6-8'];
const ALL_ATTRS = ['motif', 'color', 'fill', 'count', 'size', 'rot'];
...
// Independent series solver: continue every active progression from the VISIBLE cells
// using the documented positional grammar (does not trust the generator's own tag).
// This is the schema-spec `unique_answer` / `key_matches_solver` check for a grammar item.
function solveBlank(cells, mode, activeAttrs, rows, cols) {
  const predicted = { ...cells[0][0] };
  if (mode === 'row') {
    const bc = cols - 1;
    for (const a of activeAttrs) {
      if (LINEAR_ATTRS.includes(a)) {
        const step = cells[0][1][a] - cells[0][0][a];
        predicted[a] = cells[0][0][a] + step * bc;
      } else {
        predicted[a] = cells[0][bc % 2][a]; // period-2 cycle
      }
    }
  } else {
    const G = rows;
    for (const a of activeAttrs) {
      if (a === 'count') predicted.count = cells[0][G - 1].count; // column progression
      else if (a === 'size') predicted.size = cells[G - 1][0].size; // row progression
      else if (a === 'color') predicted.color = cells[G - 1][0].color; // row
      else if (a === 'rot') {
        const step = cells[0][1].rot - cells[0][0].rot;
        predicted.rot = cells[0][0].rot + step * (G - 1 + (G - 1));
      } else if (a === 'motif') {
        predicted.motif = cells[0][(G - 1 + (G - 1)) % 3].motif; // (r+c)%3 diagonal
      }
    }
  }
  return predicted;
...
for (const it of items) {
  ...
  if (it.typeCode !== 'FLU-CARPET-01') fail(id, `typeCode != FLU-CARPET-01 (${it.typeCode})`);
  ...
  const c = it.content || {};
  if (!['row', 'grid'].includes(c.mode)) fail(id, `mode not row|grid (${c.mode})`);
  const rows = c.rows;
  const cols = c.cols;
  const expRows = c.mode === 'grid' ? 3 : 1;
  if (rows !== expRows) fail(id, `rows ${rows} != ${expRows} for mode ${c.mode}`);
  if (c.mode === 'grid' ? cols !== 3 : !(cols >= 3)) fail(id, `cols ${cols} invalid for mode ${c.mode}`);
  if (!Array.isArray(c.activeAttrs) || c.activeAttrs.length < 1) fail(id, 'activeAttrs empty');
  else if (!c.activeAttrs.every((a) => ALL_ATTRS.includes(a))) fail(id, `bad activeAttr (${c.activeAttrs})`);

  // Carpet: rows x cols, exactly one blank at the last (bottom-right) cell.
  const cells = c.carpet && c.carpet.cells;
  if (!Array.isArray(cells) || cells.length !== rows || cells.some((r) => !Array.isArray(r) || r.length !== cols))
    fail(id, 'carpet.cells is not rows x cols');
  else {
    let nulls = 0;
    for (let r = 0; r < rows; r++) for (let cc = 0; cc < cols; cc++) if (cells[r][cc] === null) nulls++;
    if (nulls !== 1) fail(id, `expected exactly 1 blank cell, got ${nulls}`);
    if (cells[rows - 1][cols - 1] !== null) fail(id, 'blank must be the last (bottom-right) cell');
  }
  ...
  for (const o of opts) {
    ...
    for (const leak of ['lure', 'tag', 'detail', 'correct', 'isCorrect', 'attributesViolated'])
  ...
  // key_matches_solver + unique_answer: independently continue the weave and confirm
  // exactly one option matches it, and that option is the declared correctKey.
  if (Array.isArray(cells) && cells.length === rows && c.activeAttrs) {
    const expected = solveBlank(cells, c.mode, c.activeAttrs, rows, cols);
  ...
  try {
    const regen = normalizeBankItem(genItem({ mode: lev.mode, attrs: lev.attrs, distractorSimilarity: lev.distractorSimilarity, keyPosition: lev.keyPosition, seed: it.provenance.seed }));
  ...
  const derived = round2(difficultyFromLevers(lev.mode, lev.attrs || [], lev.distractorSimilarity));
...
// ---- Report ----
console.log(`FLU-CARPET-01 bank check: ${items.length} items`);
