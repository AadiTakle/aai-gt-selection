// Independent validator for the VER-SEQUENCE-01 structured bank.
//
...
//
// Run:  node research/exam-question-types/generators/check-VER-SEQUENCE-01.mjs
...
for (const it of items) {
  ...
  if (!Array.isArray(events) || events.length < 3 || events.some((e) => !e || typeof e.text !== 'string' || !e.text)) { fail(id, 'content.events must be >=3 tokens with text'); continue; }
  events.forEach((e) => { if (!deepEq(Object.keys(e).sort(), ['text'])) fail(id, `event carries extra fields (${Object.keys(e)})`); });
  if (c.eventCount !== events.length) fail(id, 'content.eventCount mismatch');
  ...
  const sigs = opts.map((o) => (o.order || []).join(','));
  if (new Set(sigs).size !== sigs.length) fail(id, 'duplicate orderings across options');
  ...
  if (Array.isArray(it.ageBands) && it.ageBands.includes('K-1')) {
    const words = events.flatMap((e) => e.text.toLowerCase().replace(/[^a-z\s]/g, ' ').split(/\s+/).filter(Boolean));
