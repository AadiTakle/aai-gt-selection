// Independent validator for the VER-POLYSEME-01 structured bank.
//
...
//
// Run:  node research/exam-question-types/generators/check-VER-POLYSEME-01.mjs

import { readFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { buildBank } from './VER-POLYSEME-01.mjs';
import { lureLabel, serializeBank } from './item-shape.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const BANK = resolve(__dirname, '../banks/VER-POLYSEME-01.jsonl');

const TYPE = 'VER-POLYSEME-01';
const DOMAIN = 'verbal';
const SCORING_MODE = 'deterministic_key';
const LURE_CLASSES = new Set(["correct","local_fit","associate","global_mismatch"]);
const OPTION_KEYS = ["picture"];
...
for (const it of items) {
  ...
  if (typeof c.word !== 'string' || !c.word) fail(id, 'content.word missing');
  if (typeof c.sentence !== 'string' || !c.sentence) fail(id, 'content.sentence missing');
  else if (c.word && !c.sentence.toLowerCase().includes(String(c.word).toLowerCase())) fail(id, `the homograph "${c.word}" must appear in the sentence`);
  ...
  opts.forEach((o, oi) => {
    ...
    if (!o.picture || typeof o.picture.text !== 'string' || !o.picture.text) fail(id, `option[${oi}] picture must have text`);
    else if (!deepEq(Object.keys(o.picture).sort(), ['text'])) fail(id, `option[${oi}] picture carries extra fields (${Object.keys(o.picture)})`);
  });
  const sigs = opts.map((o) => ((o.picture || {}).text || '').toLowerCase());
  if (new Set(sigs).size !== sigs.length) fail(id, 'duplicate picture labels');
