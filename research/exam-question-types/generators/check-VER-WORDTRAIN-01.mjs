// Independent validator for the VER-WORDTRAIN-01 structured bank.
//
...
//
// Run:  node research/exam-question-types/generators/check-VER-WORDTRAIN-01.mjs

import { readFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { buildBank } from './VER-WORDTRAIN-01.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const BANK = resolve(__dirname, '../banks/VER-WORDTRAIN-01.jsonl');

const TYPE = 'VER-WORDTRAIN-01';
const DOMAIN = 'verbal';
const SCORING_MODE = 'deterministic_key';
const LURE_CLASSES = new Set(["correct","near_order","reversed_relation","global_mismatch"]);
const OPTION_KEYS = ["order"];
...
for (const it of items) {
  ...
  const cards = c.cards;
  if (!Array.isArray(cards) || cards.length < 3 || cards.some((e) => !e || typeof e.text !== 'string' || !e.text)) { fail(id, 'content.cards must be >=3 tokens with text'); continue; }
  cards.forEach((e) => { if (!deepEq(Object.keys(e).sort(), ['text'])) fail(id, `card carries extra fields (${Object.keys(e)})`); });
  if (c.cardCount !== cards.length) fail(id, 'content.cardCount mismatch');
  if (new Set(cards.map((e) => e.text.toLowerCase())).size !== cards.length) fail(id, 'card words must be distinct');
  if (!isNum(c.syntacticComplexity)) fail(id, 'content.syntacticComplexity missing');
  ...
  opts.forEach((o, oi) => {
    ...
    if (!Array.isArray(o.order) || o.order.length !== n || new Set(o.order).size !== n || o.order.some((v) => !(v >= 0 && v < n))) fail(id, `option[${oi}] order must be a permutation of 0..${n - 1}`);
  });
  const sigs = opts.map((o) => (o.order || []).map((i) => (cards[i] || {}).text).join(' '));
  if (new Set(sigs).size !== sigs.length) fail(id, 'two options render the same sentence');
