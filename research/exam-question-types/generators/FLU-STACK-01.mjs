 ...
 * @param {{bits:number, ops:string[], distractorSimilarity:number, keyPosition:(number|Function), seed:string}} lever
 */
export function genItem({ bits, ops, distractorSimilarity, keyPosition, seed }) {
...
export function buildBank({ perBin = 6 } = {}) {
  ...
  for (let k = 1; k <= 20; k++) {
    ...
    for (let i = 0; i < perBin; i++) {
      ...
      items.push(genItem({ bits: seg.bits, ops: seg.ops, distractorSimilarity: similarity, keyPosition, seed }));
    }
  }
  return items;
}
