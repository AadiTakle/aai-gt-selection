// Independent validator for the VER-SORTBOT-01 structured bank.
//
...
//
// Run:  node research/exam-question-types/generators/check-VER-SORTBOT-01.mjs

import { readFileSync } from 'node:fs';
import { resolve, dirname } from 'node:path';
import { fileURLToPath } from 'node:url';
import { buildBank } from './VER-SORTBOT-01.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const BANK = resolve(__dirname, '../banks/VER-SORTBOT-01.jsonl');

const TYPE = 'VER-SORTBOT-01';
const DOMAIN = 'verbal';
const SCORING_MODE = 'deterministic_key';
const LURE_CLASSES = new Set(["correct","surface_match","associate","global_mismatch"]);
const OPTION_KEYS = ["token"];
...
for (const it of items) {
  ...
  if ('rule' in c) fail(id, 'content.rule is the hidden category the child must infer — it must not be served');
  const rule = ((it.provenance || {}).derivation || {}).rule;
  if (typeof rule !== 'string' || !rule) fail(id, 'provenance.derivation.rule (server-side) missing');

  // ---- type-specific content shape ----
  if (c.presentation !== 'word') fail(id, "content.presentation must be 'word' (D-017)");
  const inOk = Array.isArray(c.examplesIn) && c.examplesIn.length >= 2 && c.examplesIn.every((t) => t && typeof t.text === 'string');
  const outOk = Array.isArray(c.examplesOut) && c.examplesOut.length >= 1 && c.examplesOut.every((t) => t && typeof t.text === 'string');
  if (!inOk) fail(id, 'content.examplesIn must be >=2 tokens with text');
  if (!outOk) fail(id, 'content.examplesOut must be >=1 token with text');
  ...
  opts.forEach((o, oi) => {
    ...
    if (!o.token || typeof o.token.text !== 'string' || !o.token.text) fail(id, `option[${oi}] token must have text`);
    else if (!deepEq(Object.keys(o.token).sort(), ['text'])) fail(id, `option[${oi}] token carries extra fields (${Object.keys(o.token)})`);
  });
  const sigs = opts.map((o) => ((o.token || {}).text || '').toLowerCase());
  if (new Set(sigs).size !== sigs.length) fail(id, 'duplicate option words');
  ...
  // The keyed member must not also be shown as an OUT example.
  if (outOk && typeof (it.answer || {}).correctKey === 'number') {
    const co = opts[it.answer.correctKey];
    const outSet = new Set(c.examplesOut.map((t) => t.text.toLowerCase()));
    if (co && co.token && outSet.has(co.token.text.toLowerCase())) fail(id, 'correct option also appears in examplesOut');
  }
