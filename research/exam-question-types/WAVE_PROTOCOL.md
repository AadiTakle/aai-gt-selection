...
4. **Pick the next wave:**
   - **Breadth/fill** — if any area x band cell has `<3` types, launch targeted fill agents
     for those cells.
   - **Novelty** — if coverage is complete, launch novelty agents: each invents NEW creative
     question types (per area, plus cross-cutting game-based types that span constructs),
     **deduped** against existing `type_id`s and names in `catalog/master_types.jsonl`.
