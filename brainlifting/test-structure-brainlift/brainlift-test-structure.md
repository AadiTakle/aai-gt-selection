...
- **Spiky POV 1 - A gifted screener should measure two different constructs in two different test structures.**
   - **Elaboration:** There are two things our test should understand and measure: "What can this applicant already do?" (standing) and "How fast can they learn under difficulty" (learning-rate/timeback readiness). Unfortunately, these two measurements require different test structures to be adequately measured (Insight 1). Standing is most cleanly read when questions are asked below a student's skill/knowledge limit, while learning rate only becomes truly observable above that limit (5.8, GA 10.8). Therefore, a single test that tries to measure both will not be able to meaasure either one accurately enough for the high-stakes gate that we are measuring for. Instead, a MST that covers the standing in a first stage and then learning-rate in a second difficulty-scaled stage will provide more accurate results. The accuracy stage will be adaptive to identify where the student's limits are in each of the core reasoning areas we are looking at as well as their ability to accurately answer questions below that limit (Insight 2). Then, the learning-rate stage will use that information to scale the difficulty of its test past the student's limit so they actually struggle to answer the proposed questions. Then, through this struggle, students will demonstrate that they are able to understand their mistakes and correct/improve from them (Insight 3). This MST design also needs to account for fatigue and effort loss over the course of the test, so incorporating a break between phases and tracking real time response time data to account for engagement dropoff will make the test more effective (Insight 6).
...
- **Insight 2 - When identifying a student's reasoning limits, don't ramp up to their limits, bracket around them.** All classification stopping rules (SPRT/GLR/confidence-interval) stop the test as soon as the cut decision is statistically secure and shorter than fixed-precision estimation (1.3). The immediate instinct to measure a student's limits by pushing the difficulty until the student plateaus will provide this statistical security, but it also provides the exact conditions that raise rapid-guessing, reduced test effort, and test anxiety in kids (6.2, 7.6, 7.1, 2.1). These side effects will affect our ability to provide an adequate student ability estimate. Instead, we should bracket the student's ability and close in on their limits from both sides, that way there is no point where they feel hopelessly lost or burnt out. Luckily, this is the exact methodology that SPRT uses (1.3), making it a prime candidate for classification for the students' standings. Additionally, initially aiming to bracket around the estimated gifted cut also allows us to prevent students of varied abilities from feeling lost or anixous as a result of a initial ability estimate that was too high and shortens the test since we start on a tighter bound (protecting a low-effort child, 6.1). 
