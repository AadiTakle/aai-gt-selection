...
describe('the oracle is sound', () => {
  ...
  /**
   * RE-STATED, BECAUSE THE FINDING CHANGED UNDER MEASUREMENT RATHER THAN UNDER ARGUMENT. This test
   * was written against the 234-item bank and recorded a graded exposure: the top of the scale
   * narrowed the field to two or three options, lifting the effective chance rate there to between
   * 0.28 and 0.35 while the bottom stayed under 0.22. That is why it asserted `ceiling > 0.28`.
   *
   * The rebuilt bank closes it. Every distractor is now relabelling-reachable, so the option field no
   * longer narrows with difficulty, and the measured ceiling is exactly 0.20 — the five-option chance
   * rate, i.e. no graded leak at all. The assertion is therefore inverted rather than relaxed: what
   * is pinned now is the ABSENCE of the exposure this test was built to record, at both ends of the
   * scale, and a regression back toward a narrowed field at the top would fail it again.
   */
  it('no longer shows a graded leak at the top of the scale, and says so numerically', () => {
    ...
    // Was `> 0.28` on the 234-item bank; the rebuild takes it to the chance rate.
    expect(ceiling).toBeLessThan(0.22);
    expect(ceiling).toBeCloseTo(0.2, 2);
  });
});
...
describe('classifying a choice against what the evidence allowed', () => {
  ...
  /**
   * RE-STATED AGAINST THE REBUILT BANK, and this one could not be inverted by changing a number
   * because the rebuild removed the input it needed. As written, this test searched the bank for an
   * item placing a distractor that NO mapping can produce, then checked the classifier called that
   * choice `ruledOut` by content. On the 234-item bank such items existed and their share rose with
   * chain depth.
   *
   * The rebuild's first anti-leak property is that every distractor must be relabelling-reachable,
   * precisely so a child cannot exclude an option without having learned the mapping. That makes an
   * unreachable distractor impossible by construction, so the search now finds nothing and the old
   * assertion cannot hold on any conforming bank. What is asserted instead is that invariant, which
   * is the stronger and more current claim.
   *
   * ONE CONSEQUENCE, RECORDED RATHER THAN HIDDEN: `classifyChoice`'s `ruledOutBy: 'content'` branch
   * is no longer reachable from the shipped bank, so no bank-driven test exercises it. It is kept in
   * the classifier because a non-conforming or future bank could produce such an item, and because
   * the oracle must not silently mis-classify one if it appears. If that branch is to stay covered it
   * needs a synthetic row rather than a bank item.
   */
  it('places no distractor that content alone can exclude — every option is reachable', () => {
    const fresh = createTracker({ persistence: 'consistent' });
    const excludable = liveBank
      .map((item) => fresh.before(item) as Row)
      .map((row) => ({ row, key: row.optionKeys.find((k) => !row.contentViableKeys.includes(k)) }))
      .filter((candidate) => candidate.key !== undefined);

    expect(excludable).toHaveLength(0);
  });
