...
import styles from './spiral-ring.module.css';
...
/**
 * The GT spiral mark inside a circular progress ring. Used as the dashboard
 * centerpiece so the brand logo *is* the journey indicator — the ring fills with
 * ember as the family advances and the spiral sits at its heart.
 */
export function SpiralRing({
  ...
  return (
    <div className={styles.ring} style={{ width: size, height: size }}>
      ...
      <div className={styles.spiral} style={{ width: size * 0.4, height: size * 0.4 }}>
      ...
      {label ? <span className={styles.centerLabel}>{label}</span> : null}
    </div>
  );
}
