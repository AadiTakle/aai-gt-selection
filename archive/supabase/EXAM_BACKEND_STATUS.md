...
amended 2026-07-25 with the ownership split (§8, D-029).**
...
decisions **D-017** and **D-029** both remain *Proposed*.
...
> reasoning behind it. It was **implemented** as decision **D-029** (still *Proposed*, still
...
2. ~~**Single source of truth is unresolved** (§6).~~ **Resolved 2026-07-25 by D-029** — see §8.
...
4. ~~**The variable-length battery is effectively fixed-length under the shipped default
   policy.**~~ **Resolved 2026-07-25 by D-029** — see §8.2. The diagnosis stands exactly as
...
## 8. The ownership split (2026-07-25, decision D-029, *Proposed*)

**One migration: `supabase/migrations/20260725050000_exam_outcome_ownership.sql`.** It implements
the §6 recommendation. D-029 in `docs/governance/DECISION_LOG.md` records the decision, the
...
Nothing was dropped, so reversal is additive. See D-029 for the ratified wording; operationally:
...
- `apps/web` does not call any of these RPCs yet (§8.4 is the handoff). Verified on
  ...
  1. **The D-029 payload change breaks no caller today.** Removing `done` and `outcome` from
  ...
  2. **Answer verification is currently implemented twice, in two tiers.** The app verifies
     ...
     Postgres. This is the same duplicate-authority defect D-029 removed *within* the
