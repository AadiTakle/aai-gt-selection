-- The awkward per-type verifiers (D-027, 20260725183000_exam_verify_awkward_batch.sql) and
-- the server-only lexicon they needed, less WM-gate-01, GB-SHAPEFIT-01 and GB-PATHFORGE-01,
-- whose types were retired and whose verifiers were dropped in
...
--   3. RESOLUTION — the dispatcher routes all five surviving type codes to the ported
--      functions.
--   4. SEMANTICS — pinned verdicts and metric maps for each of the five, including the
--      FLU-GRIDCOPY-01 search agreeing with its own un-memoised transcription and the
--      SPA-PIPES-01 bug that is reproduced rather than fixed.
--
...
-- --- 3. Resolution: the dispatcher reaches all five surviving ported verifiers --------
...
select is(
  (
    ...
    where type_code in (
      'FLU-GRIDCOPY-01', 'SPA-PUNCH-01', 'SPA-TANGRAM-01',
      'SPA-PIPES-01', 'GB-WORDLADDER-01'
    )
  ),
  '{exam_verify_gridcopy,exam_verify_pipes,'
    || 'exam_verify_punch,exam_verify_tangram,exam_verify_wordladder}',
  'all five surviving type codes are registered to their ported plpgsql verifier'
);                                                                                      -- 12
select is(
...
-- --- 4b. SPA-PUNCH-01: the reverse unfold and the mirror flag -------------------------
...
-- --- 4c. SPA-TANGRAM-01: the orientation closure --------------------------------------
...
-- --- 4d. SPA-PIPES-01: the shared flood -----------------------------------------------
...
-- --- 4e. GB-WORDLADDER-01, and that no verdict leaks a lexicon word -------------------
