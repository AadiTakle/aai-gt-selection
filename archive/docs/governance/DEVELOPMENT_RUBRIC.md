...
`R2`, `R3`, and `R6` are **conditional gates** under D-400: they do not apply to
work that makes no program-effect claim, and they apply in full to work that
does. A conditional gate cannot be waived by scoping a claim vaguely — if the
output could reasonably be read as a statement about what the program causes,
the gate applies.
...
| R2 Counterfactual _(conditional)_ | Treatment and comparison are defined, comparability is justified, and remaining bias is bounded. | Identification argument and threat analysis |
| R3 Prospective causal question _(conditional)_ | Population, exposure, comparison, outcome, horizon, analysis, and maximum claim are fixed before results. | Approved protocol or specification |
| R4 Non-circularity | Selection standing is not presented as program impact; baseline and comparison roles are explicit. Selection instruments are not validated only against the signals they are meant to replace. | Selection-to-outcome data map |
| R5 Capability standard | Capability-and-fit and each decision-used measure have a defensible meaning and validity rationale. | Measure rationale, boundary cases, error handling |
| R6 Growth measurement _(conditional)_ | Baseline, high-ceiling outcome, horizon, attrition, missingness, and practice effects are addressed. | Measurement validation evidence |
...
| R11 Screening instrument | The screener's target, scale of operation, tunable cut, GT-data validation, and preserved human path are specified and evidenced. | Validation plan and parameter/cut ownership record |
...
> **Open under D-400.** These weights predate the rescope and still allocate the
> largest single share to the aim that is now non-binding, which will
> under-score screener-first concepts. They are deliberately left unchanged
> pending an owner decision, because picking replacement weights is a fresh
> judgement rather than a consequence of the rescope. See
> `RESCOPE_ANALYSIS_2026-07-30.md` §7.4.
...
- cannot map to R1–R11 or H1–H10;
