...
1a. **R11 is not invented — it is stranded (verified 2026-07-28).** `R11 — Provide a scalable, tunable, GT-validated screening instrument` is fully defined in `feat/adaptive-exam-app:docs/product/project-requirements.md`, and that branch also carries **5 R11 rows in `TRACEABILITY_MATRIX.md` and 8 in `FEATURE_TO_REQUIREMENT_MAP.md`**. No other lineage defines it: `dev`, `feat/exam-integration`, and `feat/test-structure-revamp` all stop at R10. This explains a finding that otherwise looks like sloppiness — Gen-B's exam code carries exactly one requirement citation and it reads `serves R7/R11`, because **Gen-B was built against this governance fork**, where R11 exists and is wired end to end. Transcription is therefore more tractable than "write a new requirement": the definition and both sets of traceability rows exist and can be ported. It is still **not mechanical**, because the R11 text cites `E-074` and the surrounding `E-071–E-077` range is exactly the forked evidence block described in item 3.
