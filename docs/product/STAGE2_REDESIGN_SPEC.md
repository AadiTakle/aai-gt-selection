...
**Status:** Proposed except where a section records otherwise. **§5.1 is built** — see D-211 and E-211. The clause is scoped per section deliberately: sibling workstreams build other sections behind their own flags and decisions, and each adds its own clause here rather than rewriting this one.
