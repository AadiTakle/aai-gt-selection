...
| AX-02 | IRT/CAT adaptive engine (`packages/exam-engine`) + deterministic scoring (`packages/exam-scoring`) | R11, R5, H6 | MVP-bounded | In progress | Item information, theta estimation, max-information selection, engagement gate, SE/max-item stop, exposure control; unit-tested; params synthetic/`validated=false`. D-016 named this one package `packages/cat-engine`; it is now these two. Routing also serves back-to-back bursts within one question type so a child reads an instruction once instead of once per item, caps what a type earns from tracked-inert metric coverage, and requires each area's evidence to span two types (D-201; measured in E-201 and `docs/product/EXAM_BURST_INSTRUCTION_COST.md`). The question order is drawn per session rather than replayed from one constant seed, using randomesque exposure control over the near-optimal types and items plus a recency discount that waits for the area to bracket (D-202); the type tolerance is priced against the capped coverage spread it draws from, so variety cannot outbid the enforced coverage that gates the stop rule (D-203; measured in E-203 and `docs/product/EXAM_SELECTION_INTEGRATION.md`). |
