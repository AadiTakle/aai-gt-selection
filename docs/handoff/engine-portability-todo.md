...
shapes, and **not one of them knew about `domains` or `passRoute`** — added by 1a.4 and 1a.7 the same day.
Every client had been silently discarding fields the server was returning. That is the failure mode 3.4
exists to prevent, caught in the act.

**The enforced half is the type annotations, not the spec.** The five bank handlers in `apps/api` are
annotated with the contract's response types, so a response that stops matching stops compiling. That is
the guarantee worth having. The OpenAPI document is the publishable half for adopters who are not writing
TypeScript, generated by `npm run api:spec` with a test asserting the committed copy matches the
generator. **Be precise about what that buys:** the committed file cannot fall behind the generator, but
the generator's schemas sit *beside* the TypeScript rather than being derived from it, so adding a field
to `QbankState` and forgetting the schema is still possible. A test pins the `SessionState` field list
against that specific drift. A real derivation needs a TS-to-JSON-Schema step and is worth doing if the
spec ever becomes load-bearing for someone outside this repo.

**Three things the exercise fixed rather than described:**

- **`NextResponse` is discriminated on `done`.** The lab hook declared every served field optional and
  patched the gaps at the call site (`?? ''`, `?? 0`), so a malformed response produced a question with an
  empty domain and zero information instead of an error. Narrowing removed the defaults entirely.
- **One client, one error path.** The four hand-rolled `api()` helpers handled failure differently and one
  read `res.json()` without checking `res.ok`, so a 400 arrived as an object with an `error` key and was
  rendered as a session. `BankClient` either returns the body or throws `BankApiError` with the status.
- **`lab-character` argued these types should stay local**, on the grounds that the HTTP shapes are not
  the engine's internal `QbankSessionConfig` — precision is an index there and resolved on the server.
  That was correct, and the contract answers it rather than overruling it: `CreateSessionRequest` carries
  `precisionIndex`, `ResolvedSessionConfig` is what comes back, and an app imports wire shapes without
  ever reading the engine.

**Scope.** The `/api/bank/*` family — the two questions this workstream is about. The generator screener,
the item library, snapshots and both practice modes share the process and are separate products; they are
named as out of scope in the spec rather than left ambiguous. The one remaining projection in
`App.tsx` is now a `Pick<QbankState, ...>` rather than a restatement, so it cannot drift either.

**What the spec makes explicit for a non-TypeScript adopter**, because none of it is guessable: the answer
key never crosses to the client; `correct: null` means unmarkable and is excluded from the estimate rather
than counted as wrong; `stopReason: 'confident-below'` beside `decision: 'recommend'` is valid and why; a
`DomainBand` mean must never be rendered without its interval and counts; and a domain-triggered pass is
not evidence of a domain strength. Tests assert those last three warnings are actually present in the
document, since they are the ones a reader most needs and a generator most easily loses.
