...
4. **`D-020…D-028` and `E-073…E-092`** exist only on `feat/exam-integration` and forward-port without ID collisions (they're unused on `dev`), **but** their internal evidence cross-references belong to the forked register above, so they can't be pasted blind either.
...
2. **Renumber the Gen-B collisions on merge:** `feat/exam-integration` `D-019` → `D-029`; its `E-072` → `E-093` (next free after forward-porting `E-073…E-092`). Update every in-branch reference to the old IDs.
