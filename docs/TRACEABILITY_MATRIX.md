...
| TECH-002 | Scaffold the pnpm monorepo and local development boundaries | R1, R5, R7–R10, H9, H10 | Give frontend, contracts, synthetic fixtures, and local Supabase one reproducible workspace without preempting the architecture plan | Node/pnpm versions and lockfile are pinned; workspace shells install and validate; architecture-owned files are untouched; no remote Supabase, production credential, or business logic is introduced | Technical owner | Team | In progress | None |
