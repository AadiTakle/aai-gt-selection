...
**Builds on (does not duplicate):** `docs/COGAT_GAPS_AND_TEST_SUITE_REPORT.md`, `docs/HOLISTIC_GIFTEDNESS_EVIDENCE_REPORT.md`, `docs/METRICS_AND_GUARDRAILS_LIBRARY.md`, and the annotated register of `gt-school-counterfactual-brainlift/`. Also incorporates the **measurement-quality subset** of `docs/EVIDENCE_DOSSIER.md` (origin/dev — an explicitly *unvetted* overnight research scrape); see source-register **Section 11**. Its selection/lottery/counterfactual findings (C1, C4–C7) belong to the counterfactual BrainLift, not here.
